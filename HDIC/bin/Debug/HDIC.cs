       ...
       /// <returns></returns>
        public static string byteToHexStr(byte[] bytes)
        {
            ...
            if (bytes != null)
            {
                ...
                //    returnStr += bytes[i].ToString("X2");
                //}
                returnStr += "Fuse_USB_Disable : 0x"+bytes[0].ToString("X2")+"\r\n";
                returnStr += "SecondSmartCard_Disable : 0x" + bytes[1].ToString("X2") + "\r\n";
                returnStr += "Fuse_Disable_NonDescramble_Mode : 0x" + bytes[2].ToString("X2") + "\r\n";
                returnStr += "Fuse_Disable_DVB_CSA_Mode : 0x" + bytes[3].ToString("X2") + "\r\n";
                returnStr += "Fuse_Disable_ICAM_Mode : 0x" + bytes[4].ToString("X2") + "\r\n";
                returnStr += "Fuse_DDR_Obfuscator_Enable : 0x" + bytes[5].ToString("X2") + "\r\n";
                returnStr += "Fuse_SCPU_Secure_Mode : 0x" + bytes[6].ToString("X2") + "\r\n";
                returnStr += "CPU_AccessRandomNumberRegister_Disable : 0x" + bytes[7].ToString("X2") + "\r\n";
                returnStr += "Fuse_ContentProtect_Enable : 0x" + bytes[8].ToString("X2") + "\r\n";
                returnStr += "Fuse_OTP_RegbusSecureMode_Disable : 0x" + bytes[9].ToString("X2") + "\r\n";
                returnStr += "Fuse_DMA_RegbusSecureMode_Disable : 0x" + bytes[10].ToString("X2") + "\r\n";
                returnStr += "Fuse_OTP_TestMode_Disable : 0x" + bytes[11].ToString("X2") + "\r\n";
                returnStr += "CommandMode_Disable : 0x" + bytes[12].ToString("X2") + "\r\n";
                returnStr += "RSA_Signature_Check : 0x" + bytes[13].ToString("X2") + "\r\n";
                returnStr += "RSA_Protected : 0x" + bytes[14].ToString("X2") + "\r\n";
                returnStr += "FlashCheck_Size : 0x" + bytes[15].ToString("X2") + "\r\n";
                returnStr += "SecureBoot_Enable : 0x" + bytes[16].ToString("X2") + "\r\n";
                returnStr += "RSA_Key_Select : 0x" + bytes[17].ToString("X2") + "\r\n";
                returnStr += "Fuse_DMA_Key_RAM_CPU_Write_Disable : 0x" + bytes[18].ToString("X2") + "\r\n";
                returnStr += "Fuse_TP_Key_RAM_CPU_Write_Disable : 0x" + bytes[19].ToString("X2") + "\r\n";
                returnStr += "I2S_Disable : 0x" + bytes[20].ToString("X2") + "\r\n";
                returnStr += "SPDIF_Disable : 0x" + bytes[21].ToString("X2") + "\r\n";
                returnStr += "TransportOut_Disable : 0x" + bytes[22].ToString("X2") + "\r\n";
                returnStr += "Fuse_Des3_Disable : 0x" + bytes[23].ToString("X2") + "\r\n";
                returnStr += "Fuse_RunTimeCodeCheck_Enable : 0x" + bytes[24].ToString("X2") + "\r\n";
                returnStr += "Fuse_DebugConfig_Locked : 0x" + bytes[25].ToString("X2") + "\r\n";
                returnStr += "Fuse_Memory_BIST_Mode : 0x" + bytes[26].ToString("X2") + "\r\n";
                returnStr += "Fuse_ScanChain_Mode : 0x" + bytes[27].ToString("X2") + "\r\n";
                returnStr += "Fuse_BoundaryConfig_Locked : 0x" + bytes[28].ToString("X2") + "\r\n";
                returnStr += "Fuse_DebugModeProtect_Disable : 0x" + bytes[29].ToString("X2") + "\r\n";
                returnStr += "ICAM_Config_Bit : 0x" + bytes[30].ToString("X2") + "\r\n";
            }
            return returnStr;
        }
